...
> implements AsyncIterableIterator<Record<string, AttributeValue>> {

    private _iteratedCount = 0;
    private lastResolved: Promise<IteratorResult<Record<string, AttributeValue>>> = <any>Promise.resolve();
    private readonly pending: Array<Record<string, AttributeValue>> = [];
     ...
     * @inheritDoc
     */
    [Symbol.asyncIterator](): AsyncIterableIterator<Record<string, AttributeValue>> {
        return this;
    }
     ...
     * @inheritDoc
     */
    next(): Promise<IteratorResult<Record<string, AttributeValue>>> {
     ...
     * @inheritDoc
     */
    return(): Promise<IteratorResult<Record<string, AttributeValue>>> {
    ...
    private getNext(): Promise<IteratorResult<Record<string, AttributeValue>>> {
        ...
        return this.paginator.next().then(({done, value}) => {
            if (done) {
                return {done} as IteratorResult<Record<string, AttributeValue>>;
            }
