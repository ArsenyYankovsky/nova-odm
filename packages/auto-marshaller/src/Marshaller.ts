...
export class Marshaller {
     ...
     */
    public marshallItem(item: {[key: string]: any}): Record<string, AttributeValue> {
     ...
     * JavaScript values.
     */
    public unmarshallItem(item: Record<string, AttributeValue>): UnmarshalledMapAttributeValue {
    ...
    private marshallBinaryValue(binary: BinaryValue): AttributeValue|undefined {
        if (binary.byteLength > 0 || this.onEmpty === 'leave') {
            if ((binary as ArrayBufferView).buffer) {
                return {B: new Uint8Array((binary as ArrayBufferView).buffer)};
            }

            return {B: new Uint8Array(binary as ArrayBuffer)};
        }
    ...
    private marshallObject(object: {[key: string]: any}): AttributeValue {
        return {
            M: Object.keys(object).reduce(
                (map: Record<string, AttributeValue>, key: string): Record<string, AttributeValue> => {
