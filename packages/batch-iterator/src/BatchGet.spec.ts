    ...
    for (const asyncInput of [true, false]) {
        ...
        it('should should retry unprocessed items', async () => {
            ...
            promiseFunc.mockImplementation(() => {
                ...
                const {input: { RequestItems }} = (mockDynamoDbClient.send.mock.calls.slice(-1)[0] as any)[0];
            ...
            const callCount: {[key: string]: number} = (calls as Array<Array<{ input: BatchGetItemInput }>>).reduce(
                (
                    ...
                    const keys = [];
                    const requestItems = RequestItems || {}
                    for (const table of Object.keys(requestItems)) {
                        keys.push(...(requestItems?.[table]?.Keys?.filter(x => x) || []));
                    }
