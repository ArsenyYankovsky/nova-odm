...
import {Schema} from "@nova-odm/marshaller";
import {
    AttributePath,
    between,
    equals,
...
jest.mock('@aws-sdk/client-dynamodb', () => {
    return Object.assign({ __esModule: true }, jest.requireActual('@aws-sdk/client-dynamodb'))
});

const mockWaitUntilTableExists = jest.fn(() => Promise.resolve({ state: 'SUCCESS' }));
const mockWaitUntilTableNotExists = jest.fn(() => Promise.resolve({ state: 'SUCCESS' }));
// @ts-ignore
jest.spyOn(DynamoDB, 'waitUntilTableExists').mockImplementation(mockWaitUntilTableExists);
// @ts-ignore
jest.spyOn(DynamoDB, 'waitUntilTableNotExists').mockImplementation(mockWaitUntilTableNotExists);
...
describe('DataMapper', () => {
    it('should set the customUserAgent config property on the client', () => {
        ...
        expect(client.config.customUserAgent)
            .toEqual([['nova-odm', expect.any(String)]]);
    });

    describe('#batchDelete', () => {
        const promiseFunc = jest.fn((_request: any) => Promise.resolve({
            UnprocessedItems: {}
        }));
        const mockDynamoDbClient = {
            config: {},
            send: jest.fn(promiseFunc),
        };
        ...
        for (const asyncInput of [true, false]) {
            it(
                ...
                async () => {
                    ...
                    for (let i = 0; i < 80; i++) {
                        ...
                        expected[Math.floor(i / 25)][0].input.RequestItems.foo.push({
                            DeleteRequest: {
                                Key: {
                                    fizz: {N: String(i)}
                                }
                    ...
                    expected[0].forEach((expectedItem, i) => {
                        expect(expectedItem.input).toEqual(calls[i][0].input)
                    })
                }
            );
            ...
            it('should should retry unprocessed items', async () => {
                ...
                const {calls} = mockDynamoDbClient.send.mock;
                expect(calls.length).toBe(4);
                const callCount: {[key: string]: number} = (calls as Array<Array<{ input: BatchWriteItemInput }>>).reduce(
                    (
                        keyUseCount: {[key: string]: number},
                        [{ input: {RequestItems}}],
                    ) => {
                        for (const {DeleteRequest: {Key: {fizz: {N: key}}}} of (RequestItems!.foo as any)) {
    ...
    describe('#batchGet', () => {
        const promiseFunc = jest.fn((_request) => Promise.resolve({
            UnprocessedItems: {}
        }));
        const mockDynamoDbClient = {
            config: {},
        ...
        it('should allow setting an overall read consistency', async () => {
            ...
            expect(mockDynamoDbClient.send.mock.calls).toEqual([
                [
        ...
        for (const asyncInput of [true, false]) {
            it(
                ...
                async () => {
                    ...
                    for (let i = 0; i < 325; i++) {
                        ...
                        expected[Math.floor(i / 100)][0].input.RequestItems.foo.Keys
            ...
            it('should should retry unprocessed items', async () => {
                ...
                const {calls} = mockDynamoDbClient.send.mock;
                const callCount: {[key: string]: number} = (calls as Array<Array<{ input: BatchGetItemInput }>>).reduce(
                    (
                        keyUseCount: {[key: string]: number},
                        [{ input: {RequestItems}}]
                    ) => {
                        for (const {fizz: {N: key}} of (RequestItems!.foo!.Keys as any)) {
                            if (key in keyUseCount) {
                                keyUseCount[key]++;
                            } else {
                                keyUseCount[key] = 1;
            ...
            it('should should retry unprocessed items', async () => {
                ...
                const callCount: {[key: string]: number} = (calls as Array<Array<{ input: BatchWriteItemInput }>>).reduce(
                    (
                        ...
                        for (const {PutRequest: {Item: {fizz: {N: key}}}} of (RequestItems!.foo as any)) {
                            if (key in keyUseCount) {
                                keyUseCount[key]++;
                            } else {
                                keyUseCount[key] = 1;
    ...
    describe('#createGlobalSecondaryIndex', () => {
        ...
        const mockDynamoDbClient = {
            config: {},
            send: jest.fn(() => updateTablePromiseFunc),
        };

        beforeEach(() => {
            updateTablePromiseFunc.mockClear();
            mockDynamoDbClient.send.mockClear();
            mockWaitUntilTableExists.mockClear();
        });
        ...
        it('should make and send an UpdateTable request', async () => {
            ...
            expect(mockWaitUntilTableExists.mock.calls).toEqual([
                [{ client: mockDynamoDbClient, maxWaitTime: 500 }, { TableName: 'foo' } ],
            ]);
        });
    })
    ...
    describe('#createTable', () => {
        ...
        it(
            ...
            async () => {
                ...
                expect(mockDynamoDbClient.send.mock.calls.length).toBe(1);

                expect(mockWaitUntilTableExists.mock.calls.length).toBe(0);
            }
        );
    ...
    describe('#delete', () => {
        ...
        it(
            ...
            async () => {
                ...
                expect((mockDynamoDbClient.send.mock.calls[0] as any)[0].input)
    ...
    describe('#deleteTable', () => {
        ...
        const mockDynamoDbClient = {
            config: {},
            send: deleteTablePromiseFunc,
        };

        beforeEach(() => {
            deleteTablePromiseFunc.mockClear();
            mockDynamoDbClient.send.mockClear();
            mockWaitUntilTableNotExists.mockClear();
        });
        ...
        it(
            ...
            expect(mockDynamoDbClient.send.mock.calls).toEqual([
                [ expect.objectContaining({ input: { TableName: 'foo' } })],
            ]);

            expect(mockWaitUntilTableNotExists.mock.calls).toEqual([
                [{ client: mockDynamoDbClient, maxWaitTime: 500 }, { TableName: 'foo' } ],
            ]);
        });
    });
    ...
    describe('#ensureGlobalSecondaryIndexExists', () => {
        ...
        it(
            ...
            async () => {
                ...
                expect(mockDynamoDbClient.send.mock.calls).toEqual([
                    [expect.objectContaining({ input: { TableName: tableName } })]
                ]);
    ...
    describe('#ensureTableExists', () => {
        ...
        it(
            ...
            async () => {
                ...
                expect(mockWaitUntilTableExists.mock.calls.length).toBe(1);
                expect((mapper.createTable as any).mock.calls.length).toBe(0);
            }
        );
    ...
    describe('#ensureTableNotExists', () => {
        ...
        it(
            ...
            async () => {
                ...
                expect(mockWaitUntilTableNotExists.mock.calls.length).toBe(0);
        ...
        it(
            ...
            async () => {
                ...
                expect(mockWaitUntilTableNotExists.mock.calls).toEqual([
                    [{ client: mockDynamoDbClient, maxWaitTime: 500 }, { TableName: tableName } ],
                ]);
                expect((mapper.deleteTable as any).mock.calls.length).toBe(0);
            }
        );
    ...
    describe('#parallelScan', () => {
        ...
        beforeEach(() => {
            ...
            mockDynamoDbClient.send.mockClear();
            mockDynamoDbClient.send.mockImplementation(promiseFunc);
        });
    ...
    describe('#query', () => {
        ...
        it(
            ...
            async () => {
                ...
                expect(mockDynamoDbClient.send.mock.calls[0][0].input)
                    .toMatchObject({ConsistentRead: true});
            }
        );
        ...
        describe('startKey serialization', () => {
            ...
            it('should not inject default values into the startKey', async () => {
                ...
                expect(mockDynamoDbClient.send.mock.calls[0][0].input.ExclusiveStartKey)
                    .toEqual({
                        snap: {S: 'key'},
                    });
            });
