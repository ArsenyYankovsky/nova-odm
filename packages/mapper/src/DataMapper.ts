...
import {
    AttributeDefinition,
    AttributeValue as AwsAttributeValue,
    ...
    DeleteTableCommand,
    waitUntilTableNotExists, DeleteItemCommand, GetItemCommand, PutItemCommand, UpdateItemCommand
...
export class DataMapper {
    ...
    constructor({
        ...
        client.config.customUserAgent = [['nova-odm', VERSION]];
        this.client = client;
    ...
    async createTable(
        ...
        if (TableStatus !== 'ACTIVE') {
            await waitUntilTableExists({client: this.client, maxWaitTime: 500}, {TableName})
        }
    }
    ...
    async deleteTable(valueConstructor: ZeroArgumentsConstructor<any>) {
        ...
        await waitUntilTableNotExists({ client: this.client, maxWaitTime: 500 }, {TableName})
    }
    ...
    ): AsyncIterableIterator<[string, Record<string, AwsAttributeValue>]> {
        for await (const item of items) {
...
function itemIdentifier(
    marshalled: Record<string, AwsAttributeValue>,
