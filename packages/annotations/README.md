# Nova ODM / Amazon DynamoDB DataMapper Annotations
...
with the `DataMapper` defined in `@nova-odm/mapper`. These annotations
...
import {embed} from '@nova-odm/mapper';
...
`BinarySet` or `NumberValueSet` (from the `@nova-odm/auto-marshaller`
package).
